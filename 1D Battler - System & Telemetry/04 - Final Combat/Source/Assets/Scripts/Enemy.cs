...
public class Enemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Color temp = transform.Find("Target").GetComponent<SpriteRenderer>().color;
        temp.a = 0.5f;
        transform.Find("Target").GetComponent<SpriteRenderer>().color = temp;

        float abilityFive_radius = GameObject.Find("AbilityFive").GetComponent<HeroAbility>().MaximumRadius;
        transform.Find("Ring").transform.localScale = new Vector3(abilityFive_radius * 2.0f, abilityFive_radius*2.0f, 1.0f);
    }
    ...
    void Update()
    {
        ...
        HeroAbility heroAbility = GameObject.Find("AbilityFive").GetComponent<HeroAbility>();

        // ADDED VISUALS FOR THE PLAYER - Jennifer Assid
        ...
        if (!SimControl.FastMode)
        {
            if (HitPoints <= 0.0f)
            {
                ...
                Color tmp = transform.Find("Target").GetComponent<SpriteRenderer>().color;
                tmp.a = 0.25f;
                transform.Find("Target").GetComponent<SpriteRenderer>().color = tmp;
                tmp = transform.Find("Ring").GetComponent<SpriteRenderer>().color;
                tmp.a = 0.0f;
                transform.Find("Ring").GetComponent<SpriteRenderer>().color = tmp;
            }
            else
            {
                if (AbilityOne.GetDamageDeduction() != 0.0f)
                {
                    gameObject.GetComponent<SpriteRenderer>().color = new Color(100.0f / 255.0f, 100.0f / 255.0f, 100.0f / 255.0f, 1.0f);
                }
                else
                {
    ...
    {
        //Set our X position to the correct starting position on the right side of the arena, while keeping the Y and Z the same.
    ...
    public EnemyAbility GetAbilityOne()
    {
        return AbilityOne;
    }
        ...
        if (damage != 0.0f) //Don't bother if the damage is 0
        {
            ...
            if (!SimControl.FastMode)
            {
                ...
                if (damage > 0.0f)
                {
                    ...
                    damageText.color = new Color(255, 215, 0);
                ...
                else
                {
                    ...
                    //Uses the "empty string plus number" trick to make it a string.
                    damageText.text = "+" + Mathf.Floor(damage * -1.0f);
                }
            }
        }

        if (HitPoints <= 0.0f && !SimControl.MultiGroupMode)
        {
            SimControl.EnemyDestroyed[Type]++;
        }
